<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="Services-Fees.aspx.vb" Inherits="Ram.cd.Services_Fees" %>
    ...
    <script type="text/javascript">
        function nothuman(s) {
            ...
            if (response.length == 0) {
                ...
                var tcodee = document.getElementById('countryCode');
                ...
                var msisdn = "";
                if (msis.value != "") {
                    msisdn = tcode.replace('+', '') + msis.value;
                }

                var result = PageMethods.VerifyIMEI(imri.value, msisdn,  <%=LanguageID%>, function (response) {
        ...
        function notpayment(s) {
            ...
            var ExpiryMonth = document.getElementById("cboMonth");
            var ExpiryYear = document.getElementById("cboYear");
            var CVV = document.getElementById("CVV");

            var tExpiryMonth = ExpiryMonth.options[ExpiryMonth.selectedIndex].value;
            var tExpiryYear = ExpiryYear.options[ExpiryYear.selectedIndex].value;

            if (NameOnCard.value == "") {
                swal("<%=GetContentA("Invalid Name", LanguageID)%>.");
                return false;
            }
            if (CardNumber.value == "") {
                swal("<%=GetContentA("Invalid Card Number", LanguageID)%>.");
                return false;
            }
            if (tExpiryMonth == "") {
                swal("<%=GetContentA("Invalid Expiry Month", LanguageID)%>.");
                return false;
            }
            if (tExpiryYear == "") {
                swal("<%=GetContentA("Invalid Expiry Year", LanguageID)%>.");
                return false;
            }
            if (CVV.value == "") {
                swal("<%=GetContentA("Invalid CVV", LanguageID)%>.");
                return false;
            }
            if (msis.value == "") {
            ...
            var result = PageMethods.ProcessPayment(IMEI.value, '', tExpiryMonth, tExpiryYear, NameOnCard.value, CardNumber.value, CVV.value, '', msisdn,  <%=LanguageID%>,
                function (response) {
                    if (response == "ERROR") {
                        swal("<%=GetContentA("Error", LanguageID)%>");
                    } else if (response == "INSUFFICIENT_FUNDS") {
                        swal("<%=GetContentA("Payment Declined Insufficient Funds", LanguageID)%>");
                    } else if (response == "DECLINED") {
                        swal("<%=GetContentA("Payment Declined", LanguageID)%>");
                    } else if (response == "SUCCESS") {
                        swal("<%=GetContentA("Payment Success", LanguageID)%>");
                    } else if (response == "INCORRECT_CVV") {
                        swal("<%=GetContentA("INCORRECT_CVV", LanguageID)%>");
                    } else if (response == "INVALID") {
                        swal("<%=GetContentA("INVALID", LanguageID)%>");
                    } else if (response == "DECLINED_EXPIRY_DATE_INCORRECT") {
                        swal("<%=GetContentA("INVALID_EXPIRY_DATE", LanguageID)%>");
                    } else {
                        swal("<%=GetContentA("No Response", LanguageID)%>");
                    }
                }
            )
        }
...
<body>
    <form id="form1" runat="server">
        <asp:ScriptManager runat="server" ID="scrp" EnablePageMethods="true"></asp:ScriptManager>
            ...
            <div class="container-full">
                <div class="row no-gutters">
                    <div class="col-md-3 col-lg-2">
                        ...
                        <%--  <div class="vry-left" style="padding-right: 0px;">
                            ...
                            <a href="Services-Fees.aspx">
                                <div class="vry-a2" style="background-color: #006AF0;">
                                    <img src="img/CheckRAMFees_selected.png" alt="images not found" />
                                    <p style="color: #fff;"><%=GetContent("Services_Menu_CheckRAMFees") %></p>
                                </div>
                            </a>
                    ...
                    <div class="col-md-9 col-lg-10">
                        ...
                        <div class="vry-right" style="padding-left: 0px;">
                            <div class="vry-b1" style="min-height: 240px">
                                <p style="margin-top: 1px;"><%=GetContent("Services_Fee_Hello") %>&nbsp;</p>
                                <div class="row no-gutters">
                                    <div class="col-lg-5 col-md-5">
                                        <input class="forminput" maxlength="15" type="number" name="txtIMEI" id="txtIMEI" placeholder="<%=GetContent("IMEI Number") %>" />*
                                    </div>
                                    <div class="col-lg-5 col-md-5">
                                        <select style="padding-left: 2px; padding-right: 0; width: 30%; <%=IIf(LanguageID = 2, "visibility:hidden;display:none;", "") %>" class="forminput input-block-level" id="countryCode<%=IIf(LanguageID = 2, "NotUsed", "") %>" name="countryCode<%=IIf(LanguageID = 2, "NotUsed", "") %>">
                                            ...
                                            <option data-countrycode="CD" value="+243" selected="selected">+243</option>
                                        ...
                                        <select style="width: 30%; padding-left: 2px; padding-right: 0; <%=IIf(LanguageID = 1, "visibility:hidden;display:none;", "") %>" class="forminput input-block-level" id="countryCode<%=IIf(LanguageID = 1, "NotUsed", "") %>" name="countryCode<%=IIf(LanguageID = 1, "NotUsed", "") %>">
                                        ...
                                        <input style="width: 68%;" class="forminput" type="number" min="0" value="" on name="txtPhone" id="txtPhone" placeholder="<%=GetContent("Phone Number") %>" />
                                    </div>
                                    <div class="col-lg-2 col-md-2">
                                        <div class="px-contact-input" align="right">

                                            <button id="btnBlock" class="submit float-right" type="button" onclick="return nothuman();">
                                                <i class="fa fa-angle-right" style="color: #e3c600"></i>&nbsp;<%=GetContent("Check") %>
                                            </button>
                                        </div>

                                        <%--                                            <button class="submit forminputbutton" type="button" runat="server" id="btnBlock" onclick="return nothuman(this);">
                                                >
                                            </button>--%>
                                        <div class="checkbox-part" style="max-height: 33px; position: inherit !important; float: right; margin-right: 0px;">
                            ...
                            <div class="degg_top-area">
                                ...
                                <br />
                                <div class="container" id="paymentTab" name="paymentTab" style="display: none;">
                                    <table class='table table-sm table-borderless'>
                                        <tr>
                                            <td class='devices_b1 whitefont'><%=GetContent("NameOnCard") %></td>
                                            <td class='devices_b2 whitefont'>
                                                <input class="forminput" type="text" value="" name="NameOnCard" id="NameOnCard" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class='devices_b1 whitefont'><%=GetContent("CardNumber") %></td>
                                            <td class='devices_b2 whitefont'>
                                                <input class="forminput" type="number" value="" name="CardNumber" id="CardNumber" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class='devices_b1 whitefont'><%=GetContent("Expiry") %></td>
                                            <td class='devices_b2 whitefont'>
                                                <select style="width: 25%;" class="forminput input-block-level" id="cboMonth" name="cboMonth">
                                                    <option value=""><%=GetContent("Month")%></option>
                                                    <option value="01"><%=GetContent("Jan")%></option>
                                                    <option value="02"><%=GetContent("Feb")%></option>
                                                    <option value="03"><%=GetContent("Mar")%></option>
                                                    <option value="04"><%=GetContent("Apr")%></option>
                                                    <option value="05"><%=GetContent("May")%></option>
                                                    <option value="06"><%=GetContent("Jun")%></option>
                                                    <option value="07"><%=GetContent("Jul")%></option>
                                                    <option value="08"><%=GetContent("Aug")%></option>
                                                    <option value="09"><%=GetContent("Sep")%></option>
                                                    <option value="10"><%=GetContent("Oct")%></option>
                                                    <option value="11"><%=GetContent("Nov")%></option>
                                                    <option value="12"><%=GetContent("Dec")%></option>
                                                </select>
                                                /
                                                    <select style="width: 20%;" class="forminput input-block-level" id="cboYear" name="cboYear">
                                                        <option value=""><%=GetContent("Year")%></option>
                                        ...
                                        <tr>
                                            <td class='devices_b1 whitefont'><%=GetContent("CVV") %></td>
                                            <td class='devices_b2 whitefont'>
                                                <input class="forminput" style="width: 140px;" type="number" value="" name="CVV" id="CVV" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="2" align="center">
                                                <button class="submit forminputbutton" type="button" runat="server" id="Button1" onclick="return notpayment(this);">
                                                    ><%=GetContent("Submit") %>
                                                </button>
                                            </td>
                                        </tr>
